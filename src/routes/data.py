...
async def upload_data(
    ...
    file_path, file_id = data_controller.generate_unique_filepath(
        project_id=project_id, original_file_name=file.filename
    )
    try:
...
async def process_endpoint(request: Request, project_id: int,
    ...
    if len(project_file_ids) == 0:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"signal": ResponseSignals.NO_FILES_ERROR.value},
        )
    inserted_count = 0
    no_files = 0
    ...
    for asset_id, file_id in project_file_ids.items():
        ...
        if file_content is None:
            ...
            continue
        processed_chunks = process_controller.process_file_content(
        ...
        inserted_count += await chunk_model.insert_many_chunks(
            chunks=processed_chunks_records
        )
        no_files += 1
    if inserted_count == 0:
        return JSONResponse(
